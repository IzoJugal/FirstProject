...
const ContactsPage: React.FC = () => {
  ...
  const totalPages: number = Math.ceil(contacts.length / pageSize);
  ...
  const fetchContacts = useCallback(async () => {
    try {
      const res = await axios.get<{ contacts: ContactType[] }>(
        `${import.meta.env.VITE_BACK_URL}/admin/contacts`,
        {
  ...
  const deleteContact = async (id: string) => {
    try {
      await axios.delete(
        `${import.meta.env.VITE_BACK_URL}/admin/contact/${id}`,
        {
          headers: { Authorization: authorizationToken },
        }
      );
