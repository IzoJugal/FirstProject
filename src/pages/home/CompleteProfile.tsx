...
interface AuthContextType {
  storeToken: (token: string, user: User, roles: string[]) => void;
}

const CompleteProfile: React.FC = () => {
  const navigate = useNavigate();
  const { id: userId } = useParams<{ id: string }>();
  const { storeToken } = useAuth() as AuthContextType;
  ...
  const [loading, setLoading] = useState<boolean>(false); // for submit
  const [initialLoading, setInitialLoading] = useState<boolean>(true); // for fetch
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (name === "roles") {
      setFormData((prev) => ({
        ...prev,
        roles: [value], // Single role selection for radio buttons
      }));
    } else {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const data: { token: string; user: User } | ApiError = await res.json();
      if (!res.ok) {
        throw new Error(
          (data as ApiError).message || "Profile completion failed"
        );
      }

      const successData = data as { token: string; user: User };
      const { token, user } = successData;
