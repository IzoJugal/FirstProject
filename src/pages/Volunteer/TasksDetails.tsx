...
interface Task {
  ...
  myVolunteerStatus: keyof typeof myVolunteerStatus;
  createdAt: string;
}
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 px-4">
      <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm text-gray-700">
          ...
          <p>
            <span className="font-semibold">Address:</span> {task.address || "N/A"}
          </p>
          ...
          <p className="sm:col-span-2">
            ...
            <span
              className={`inline-block px-2 py-1 rounded-full text-xs font-medium capitalize ${
                myVolunteerStatus[task.myVolunteerStatus]
              }`}
            >
...
const TasksDetails: React.FC = () => {
  ...
  const [loadingTaskId, setLoadingTaskId] = useState<string | false>(false);
  const [currentPage, setCurrentPage] = useState<number>(1);

  const fetchTasks = useCallback(async (): Promise<void> => {
    try {
      const res = await axios.get<{ tasks: Task[] }>(
        `${import.meta.env.VITE_BACK_URL}/auth/tasks`,
        {
          headers: {
  ...
  const handleAction = async (id: string, action: "accept" | "reject"): Promise<void> => {
    setLoadingTaskId(id);
    try {
