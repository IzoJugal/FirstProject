...
const LogoUpload = () => {
  ...
  }, [logoUrl, authorizationToken,fetchLogo]);
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(`${import.meta.env.VITE_BACK_URL}/logo/upload`, formData, {
        headers: {
      ...
      setMessage(
        err.response?.status === 401
          ? err.response.data?.error?.toLowerCase().includes('user info')
  ...
  return (
    <div className="max-w-md mx-auto p-6 bg-white rounded-lg shadow-md">
      ...
      {message && (
        <p
          className={`mt-4 text-center font-medium ${message.toLowerCase().includes('failed') || message.toLowerCase().includes('unauthorized')
              ? 'text-red-600'
              : 'text-green-600'
            }`}
        >
