...
const VolunteersData: React.FC = () => {
  ...
  const filteredVolunteers = users.filter((user) => {
    const roles = user.roles || [];
    // Check if roles are exactly ["volunteer"]
    const isOnlyVolunteer = roles.length === 1 && roles.includes("volunteer");
    // Check if roles are exactly ["user", "volunteer"] or ["volunteer", "user"]
    const isUserAndVolunteerOnly =
      roles.length === 2 &&
      roles.includes("user") &&
      roles.includes("volunteer");
    return isOnlyVolunteer || isUserAndVolunteerOnly;
  });

  // Filter the volunteers based on the search query
  const filteredUsers = filteredVolunteers.filter((u) => {
  ...
  return (
    <div className="p-4 max-w-4xl mx-auto">
          ...
          >
            ...
            <div className="flex-1">
              ...
              <p className="text-green-600 text-sm font-medium mt-1">
                ...
                {user.roles.includes("user") && user.roles.includes("volunteer") && " & "}
      ...
      {paginatedUsers.length === 0 && (
        <p className="text-center text-gray-400 text-sm">No volunteers found.</p>
      )}
