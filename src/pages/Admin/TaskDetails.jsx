...
const VolunteerSelect = memo(({ value, onChange, volunteers, error }) => {
...
const StatusSelect = memo(({ value, onChange, error, disabled }) => (
  <div className="flex flex-col">
...
// TaskCard component
const TaskCard = memo(({ task, onEdit, onDelete, isDeleting }) => {
  const status = task.status || "pending";
  // Check if task date is in the past
  const isPastDate = task.date && new Date(task.date) < new Date(new Date().setHours(0, 0, 0, 0));
  ...
  return (
    <div
      ...
      role="button"
      aria-label={isPastDate ? `Task ${task.taskTitle} (past task, cannot be edited)` : `Edit task ${task.taskTitle}`}
      title={isPastDate ? "Cannot edit past tasks" : undefined}
    >
      <div className="flex justify-between items-center">
        <h2 className="text-lg font-semibold text-blue-700">{task.taskTitle}</h2>
        <button
          onClick={(e) => {
            e.stopPropagation();
            onDelete(task);
          ...
          <ul className="list-disc pl-4">
            {task.volunteers.map((vol) => (
              <li key={vol.user?._id || Math.random()}>
                ...
                >
                  {vol.status ? vol.status.charAt(0).toUpperCase() + vol.status.slice(1) : "N/A"}
                </span>
                )
              </li>
            ))}
      ...
      <p className="text-sm text-gray-700">
        ...
        {task.date ? new Date(task.date).toLocaleDateString("en-GB") : "N/A"} at {task.time || "N/A"}
      </p>
      ...
      {isPastDate && (
        <p className="text-xs text-red-600 mt-2">This task is in the past and cannot be edited.</p>
      )}
    </div>
  );
});
...
const TaskDetails = () => {
  ...
  const [modalFormData, setModalFormData] = useState(initialFormData);
  ...
  const fetchTasks = useCallback(
    async () => {
      setLoading(true);
      try {
        const res = await axios.get(`${import.meta.env.VITE_BACK_URL}/admin/tasks`, {
  ...
  const fetchVolunteers = useCallback(async () => {
    try {
      const res = await axios.get(`${import.meta.env.VITE_BACK_URL}/admin/volunteers`, {
        headers: { Authorization: authorizationToken },
      });
  ...
  const handleDeleteTask = useCallback(
    async (taskId) => {
      ...
      try {
        await axios.delete(`${import.meta.env.VITE_BACK_URL}/admin/task/${taskId}`, {
          headers: { Authorization: authorizationToken },
        });
  ...
  const validateForm = useCallback(() => {
    const errors = {};
    const requiredFields = ["taskTitle", "taskType", "description", "date", "time", "address", "status"];
    ...
    if (modalFormData.status && !["pending", "active", "completed", "cancelled"].includes(modalFormData.status)) {
  ...
  const handleCreateTask = useCallback(async () => {
    ...
    try {
      ...
      toast.error(err.response?.data?.message || `Failed to create task: ${err.message}`);
    } finally {
      setIsCreating(false);
    }
  ...
  const handleEditTask = useCallback(async () => {
    ...
    try {
      ...
      toast.error(err.response?.data?.message || `Failed to update task: ${err.message}`);
    } finally {
      setIsEditing(false);
    }
  ...
  const openEditModal = (task) => {
    setModalFormData({
      ...
      time: task.time || "",
      volunteer: task.volunteers ? task.volunteers.map((vol) => vol.user?._id).filter(Boolean) : [],
  ...
  return (
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {displayTasks.map((task) => (
            <TaskCard
              ...
              isDeleting={taskLoading[task._id]}
            />
          ))}
        </div>
      )}
   ...
   <button
            ...
            <h2 id="modal-title" className="text-xl font-bold text-blue-700 mb-4">
              {modalMode === "create" ? "Create Volunteer Task" : "Edit Volunteer Task"}
            </h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="flex flex-col">
                ...
                <DatePicker
                  ...
                  minDate={new Date()}
                  className={`w-full px-3 py-2 border rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 outline-none ${
                    formErrors.date || formErrors.time ? "border-red-500" : "border-gray-300"
                  }`}
                  placeholderText="Select date and time"
                />
                {(formErrors.date || formErrors.time) && (
                  <p className="text-red-500 text-xs mt-1">{formErrors.date || formErrors.time}</p>
                )}
              </div>
              <VolunteerSelect
              ...
              <StatusSelect
                ...
                disabled={["completed", "cancelled"].includes(modalFormData.status)}
              />
            </div>
            ...
            <div className="flex justify-end mt-6 gap-3">
              ...
              {modalFormData.status !== "completed" && (
                <button
                  onClick={modalMode === "create" ? handleCreateTask : handleEditTask}
                  ...
                  aria-label={modalMode === "create" ? "Create task" : "Update task"}
                >
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-sm w-full shadow-xl">
            ...
            <p className="text-sm text-gray-600 mb-6">
              ...
              <span className="font-medium text-red-600">{taskToDelete?.taskTitle}</span>?
            </p>
