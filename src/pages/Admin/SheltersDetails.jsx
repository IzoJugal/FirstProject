...
import { ArrowLeftIcon, CameraIcon, FrownIcon, Loader2, Pencil, Trash2, Trash2Icon } from "lucide-react";
import { toast } from "react-toastify";
import { useAuth } from "../../authContext/Auth";
import { useNavigate } from "react-router-dom";

const SheltersDetails = () => {
    ...
    const fetchShelters = useCallback(async () => {
        try {
            const res = await axios.get(`${import.meta.env.VITE_BACK_URL}/admin/shelters`, {
                headers: { Authorization: authorizationToken },
            });
            setShelters(res.data.shelters);
        } catch (err) {
            toast.error("Failed to load shelters", err);
        }
    }, [authorizationToken]);
    ...
    const createShelter = async () => {
        // Basic validation
        if (!newShelter.name || !newShelter.phone || !newShelter.address || !newShelter.email) {
        ...
        try {
            ...
            if (res.data.success) {
                setShelters((prev) => [res.data.shelter, ...prev]);
    ...
    const updateDetails = async () => {
        ...
        try {
            ...
            formData.append("capacity", editShelter.capacity || 0);
            formData.append("currentOccupancy", editShelter.currentOccupancy || 0);


            if (editShelter.profileImage instanceof File) {
                formData.append("profileImage", editShelter.profileImage);
            }
            ...
            setShelters((prev) =>
                prev.map((s) =>
                    s._id === editShelter._id ? res.data.shelters : s
                )
            );
        ...
        <div className="p-4 max-w-5xl mx-auto">
            ...
            <div className="space-y-4">
                {paginatedShelters.length === 0 ? (
                    <div className="text-center py-10 text-gray-500">
                        ...
                        <p className="text-lg text-red-600 animate-bounce font-semibold">No shelters found.</p>
                    </div>
                ) : (
                            ...
                            <img
                                src={
                                    shelter.profileImage?.startsWith("http")
                                        ...
                                        : shelter.profileImage
                                            ? `${import.meta.env.VITE_BACK_URL}/auth/profile/image/${shelter.profileImage}`
                            ...
                            <div className="flex-1 space-y-2">
                                <div className="flex justify-between items-center">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <div
                                            onClick={() => toggleActive(shelter._id, shelter.isActive)}
                                            className={`w-10 h-5 flex items-center rounded-full px-1 cursor-pointer transition-colors ${shelter.isActive ? "bg-green-400" : "bg-red-400"}`}
                                        >
                                            <div
                                                className={`w-4 h-4 rounded-full transition-all duration-300 ${shelter.isActive ? "bg-green-800 ml-auto" : "bg-red-700"}`}
                                            />
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="text-sm text-gray-700 space-y-1">
                                    ...
                                    {shelter.contactPerson && (
                                        <p>
                                            👤 <span className="font-medium">Contact:</span> {shelter.contactPerson}
                                        </p>
                                    )}
                                </div>
                                ...
                                <p className="text-sm">
                                    ...
                                    <span
                                        className={`ml-1 font-semibold ${shelter.currentOccupancy >= shelter.capacity ? "text-red-600" : "text-green-700"}`}
                                    >
                                        {shelter.currentOccupancy}/{shelter.capacity || "N/A"}
                                    </span>
                                </p>
                            </div>
                ...
                <div className="fixed inset-0 bg-black bg-opacity-30 flex justify-center items-center z-50">
                    <div className="bg-white rounded-2xl shadow-lg p-6 w-full max-w-md text-center">
                        <div className="relative w-fit mx-auto mb-6">
                            <img
                                src={
                                    editShelter.profileImage instanceof File
                                        ? URL.createObjectURL(editShelter.profileImage)
                                        : editShelter.profileImage
                                            ? editShelter.profileImage.startsWith("http")
                                                ? editShelter.profileImage
                                                : `${import.meta.env.VITE_BACK_URL}/auth/profile/image/${editShelter.profileImage}`
                                            : "img/profile-image.webp"
                                }
                                alt="Shelter Profile"
                            ...
                            >
                                <input
                                    ...
                                    accept="image/*"
                                    onChange={(e) =>
                                        setEditShelter({ ...editShelter, profileImage: e.target.files[0] })
                                    }
                                    className="hidden"
                                />
                        ...
                        <div className="space-y-3 text-left">
                            ...
                            <input
                                ...
                                onChange={(e) =>
                                    setEditShelter({ ...editShelter, contactPerson: e.target.value })
                                }
                                className="w-full border px-3 py-2 rounded-md"
                            ...
                            <input
                                ...
                                onChange={(e) =>
                                    setEditShelter({ ...editShelter, currentOccupancy: e.target.value })
                                }
                                className="w-full border px-3 py-2 rounded-md"
                        ...
                        <div className="mt-6 flex justify-center gap-4">
                            ...
                            <button
                                ...
                                disabled={loading}
                                className={`px-6 py-2 rounded text-white transition ${loading ? "bg-green-400 cursor-not-allowed" : "bg-green-600 hover:bg-green-700"
                                    }`}
                            >
                                {loading ? (
                ...
                <div className="fixed inset-0 bg-black bg-opacity-30 flex justify-center items-center z-50">
                    <div className="bg-white rounded-2xl shadow-lg p-6 w-full max-w-md text-center">
                        ...
                        <div className="relative mx-auto mb-4">
                            ...
                            />
                            <label htmlFor="newShelterProfile" className="absolute -bottom-2 right-[35%] bg-green-600 p-2 rounded-full cursor-pointer">
                                <input
                                    id="newShelterProfile"
                                    type="file"
                                    accept="image/*"
                                    onChange={(e) =>
                                        setNewShelter({ ...newShelter, profileImage: e.target.files[0] })
                                    }
                                    className="hidden"
                                />
                                <CameraIcon size={18} className="text-white" />
                        ...
                        <div className="space-y-3 text-left">
                            <input type="text" value={newShelter.name} onChange={(e) => setNewShelter({ ...newShelter, name: e.target.value })} className="w-full border px-3 py-2 rounded-md" placeholder="Shelter Name" />
                            <input type="text" value={newShelter.contactPerson} onChange={(e) => setNewShelter({ ...newShelter, contactPerson: e.target.value })} className="w-full border px-3 py-2 rounded-md" placeholder="Contact Person" />
                            <input type="text" value={newShelter.phone} onChange={(e) => setNewShelter({ ...newShelter, phone: e.target.value })} className="w-full border px-3 py-2 rounded-md" placeholder="Phone" />
                            <input type="email" value={newShelter.email} onChange={(e) => setNewShelter({ ...newShelter, email: e.target.value })} className="w-full border px-3 py-2 rounded-md" placeholder="Email" />
                            <input type="text" value={newShelter.address} onChange={(e) => setNewShelter({ ...newShelter, address: e.target.value })} className="w-full border px-3 py-2 rounded-md" placeholder="Address" />
                            <input type="number" value={newShelter.capacity} onChange={(e) => setNewShelter({ ...newShelter, capacity: e.target.value })} className="w-full border px-3 py-2 rounded-md" placeholder="Capacity" />
                        </div>

                        <div className="mt-6 flex justify-center gap-4">
                            <button onClick={() => setShowCreateModal(false)} className="px-4 py-2 bg-gray-200 rounded hover:bg-gray-300">Cancel</button>
                            <button
                                onClick={createShelter}
                                disabled={cloading}
                                className={`px-6 py-2 rounded text-white transition ${cloading ? "bg-green-400 cursor-not-allowed" : "bg-green-600 hover:bg-green-700"
                                    }`}
                            >
                                {cloading ? (
                                    <span className="flex items-center gap-2">
            ...
            {showDeleteModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
                    <div className="bg-white p-6 rounded-lg shadow-md max-w-md w-full text-center">
                        ...
                        <p className="mb-6 text-gray-700">Are you sure you want to delete this shelter?</p>
            ...
            <div className="mt-6 flex justify-center items-center gap-2">
                {Array.from({ length: totalPages }, (_, i) => (
                    <button
                        key={i}
                        className={`w-8 h-8 rounded-full text-sm font-medium transition ${currentPage === i + 1 ? "bg-green-600 text-white" : "bg-gray-200 text-gray-700 hover:bg-gray-300"}`}
