...
const GreenSortsSignup: React.FC = () => {
  ...
  const handleVerifyOtp = async (e: React.MouseEvent<HTMLButtonElement>): Promise<void> => {
    ...
    try {
      ...
      if (res.data.token) {
        toast.success("Signup successful! You've successfully signed up as a GreenSorts.", {
          autoClose: 3000,
        });
            ...
            {["firstName", "lastName"].map((field) => (
              <div className="relative w-1/2" key={field}>
                <input
                  ...
                  className={`w-full px-4 py-3 pr-10 rounded-xl border text-sm placeholder-gray-400 bg-white focus:outline-none focus:ring-2 focus:ring-green-500 ${
                    errors[field as keyof Errors] ? "border-red-500 ring-red-300" : "border-gray-300"
                  }`}
                />
                <FaUser className="absolute right-3 top-1/2 -translate-y-1/2 text-green-600" />
                {errors[field as keyof Errors] && (
                  <p className="text-xs text-red-500 mt-1">{errors[field as keyof Errors]}</p>
                )}
              </div>
            ))}
          </div>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50 px-4">
            <div className="bg-white rounded-2xl p-6 w-full max-w-sm space-y-6 shadow-xl">
              ...
              <div className="text-center space-y-1">
                <img src="/logo.png" alt="GreenSorts Logo" className="w-12 h-12 mx-auto" />
