...
import { Home, Settings, LogOut, LayoutDashboard, ClipboardPenLine, History, ContactRound, BellIcon, BellRingIcon, PackagePlus, PackageCheck, RecycleIcon, RefreshCcw } from "lucide-react";
...
// Define types for useNotification hook
...
const Sidebar: React.FC = () => {
  const { authorizationToken, logout, user } = useAuth() as AuthContextType;
  const { unreadCount } = useNotification() as NotificationContextType;
  ...
  const handleLogout = async () => {
    try {
      ...
      console.log("✅ Logout successful, session invalidated in backend");
    } catch (error) {
  ...
  const navItems: NavItem[] = [
    ...
    { name: "Proccess Data", path: "/processedData", icon: <RefreshCcw size={20} />, roles: ["recycler"] },
              ...
              return (
                <Link
                  ...
                  className={`flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition ${
                    isActive ? "bg-[#EEF6ED] text-[#579B52] font-semibold" : "text-gray-700 hover:bg-gray-100"
                  }`}
                >
                  {isNotification ? (
