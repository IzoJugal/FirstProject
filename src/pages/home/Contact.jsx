...
export default function Contact() {
  ...
  const handleChange = (e) => {
    ...
    // Clear the error message for the field as user types
    if (errors[name]) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post(`${import.meta.env.VITE_BACK_URL}/contact/contact`, formData);
      ...
      <div className='bg-[#F6F6F6] rounded-[1.125rem]'>
        <div className='p-4 md:p-10 flex flex-col md:flex-row gap-5 md:gap-8'>
          ...
          <div className='md:w-1/2'>
            <img loading="lazy" className='rounded-[1.125rem]' src="/contact.png" alt="Contact" />
          </div>
          ...
          <form className='md:w-1/2 flex flex-col gap-4' onSubmit={handleSubmit}>
            ...
            <button
              type="submit"
              disabled={loading}
              className={`bg-[#579B52] text-white py-[2.5%] rounded-[1.125rem] cursor-pointer hover:bg-[#40753c] transition-all duration-100 ${loading && "opacity-50 cursor-not-allowed"}`}
            >
