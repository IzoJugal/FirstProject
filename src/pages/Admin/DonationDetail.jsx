...
import { ArrowLeft, CalendarDays, ChevronLeft, Clock, IndianRupee, MapPin, Phone, Wallet, WalletCards, Weight } from 'lucide-react';
...
const DonationDetail = () => {
    ...
    useEffect(() => {
        const fetchDonation = async () => {
            try {
                const res = await axios.get(`${import.meta.env.VITE_BACK_URL}/admin/pickups/${id}`, {
    ...
    const fetchDealers = async () => {
        try {
            const res = await axios.get(`${import.meta.env.VITE_BACK_URL}/admin/dealers`, {
                headers: { Authorization: authorizationToken },
            });
    ...
    const handleReject = async () => {
        try {
            await axios.post(
                `${import.meta.env.VITE_BACK_URL}/admin/reject-donation/${donation._id}`,
                {},
                { headers: { Authorization: authorizationToken } }
            );
    ...
    // You can customize this if you add timestamps later
        ...
        <div className="max-w-xl mx-auto p-4 space-y-6">
            ...
            <div className="space-y-1">
                ...
                <div className="flex items-center gap-4 text-gray-600 mt-1">
                    ...
                    {formatDate(new Date(donation.pickupDate))}
                </div>
            </div>
                ...
                <div className="rounded-xl p-4 shadow-sm">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="flex items-center justify-between border border-gray-300 rounded-md px-3 py-2 text-green-700 font-semibold text-lg">
                            <span>{donation.price?.toLocaleString() ?? '0'}</span>
                            <Wallet size={20} />
                        </div>
