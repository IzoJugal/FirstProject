...
const DealersData = () => {
    ...
    const fetchDealers = useCallback(async () => {
        try {
            const res = await axios.get(`${import.meta.env.VITE_BACK_URL}/admin/getdealers`, {
                headers: {
            ...
            toast.error("Failed to load dealers", err);
        }
    }, [authorizationToken]);
    ...
    const deleteDealer = async (id) => {
        try {
            await axios.delete(`${import.meta.env.VITE_BACK_URL}/admin/dealer/${id}`, {
                headers: {
                    Authorization: authorizationToken,
                }
            });
    ...
    const updateDetails = async () => {
        try {
            ...
            toast.error("Failed to update dealer", err);
        }
    };

    useEffect(() => {
        fetchDealers();
    }, [authorizationToken, fetchDealers]);
                ...
                {paginatedDealer.map((dealer) => (
                    ...
                    >
                        ...
                        <div className="flex-1">
                            <p className="font-semibold">{dealer.firstName} {dealer.lastName}</p>
                            ...
                            <div className="flex items-center gap-2 mt-1">
                                ...
                                <div
                                    ...
                                    className={`w-10 h-5 flex items-center rounded-full px-1 cursor-pointer ${dealer.isActive ? "bg-green-300" : "bg-red-300"
                                        }`}
                                >
                                    <div
                                        className={`w-4 h-4 rounded-full transition-all ${dealer.isActive ? "bg-green-700 ml-auto" : "bg-red-600"
                                            }`}
                                    ></div>
                                </div>
                            </div>
                ...
                <div className="fixed inset-0 bg-black bg-opacity-30 flex justify-center items-center z-50">
                    <div className="bg-white rounded-2xl shadow-lg p-6 w-full max-w-sm relative text-center">
                        <div className="relative mx-auto mb-4">
                            ...
                            <label htmlFor="profileImageUpload" className="absolute -bottom-2 right-[35%] bg-green-600 p-2 rounded-full cursor-pointer">
                                <input
                                    id="profileImageUpload"
                                    type="file"
                                    accept="image/*"
                                    onChange={(e) =>
                                        setEditDealer({ ...editDealer, profileImage: e.target.files[0] })
                                    }
                                    className="hidden"
                                />
