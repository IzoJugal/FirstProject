...
const GaudaanById = () => {
    ...
    const [gaudaan, setGaudaan] = useState(null);
    ...
    useEffect(() => {
        const fetchGaudaan = async () => {
            try {
                const res = await axios.get(`${import.meta.env.VITE_BACK_URL}/admin/gaudaan/${id}`, {
    ...
    if (gaudaan.status === 'shelter' || gaudaan.status === 'dropped') {
        statusList.push({ label: 'Assigned', date: gaudaan.statusHistory.find(h => h.status === 'assigned')?.timestamp || gaudaan.createdAt });
        statusList.push({ label: 'Picked up', date: gaudaan.statusHistory.find(h => h.status === 'picked_up')?.timestamp || gaudaan.updatedAt });
        statusList.push({ label: 'Donated', date: gaudaan.statusHistory.find(h => h.status === 'shelter' || h.status === 'dropped')?.timestamp || gaudaan.updatedAt });
    } else if (gaudaan.status === 'picked_up') {
        statusList.push({ label: 'Assigned', date: gaudaan.statusHistory.find(h => h.status === 'assigned')?.timestamp || gaudaan.createdAt });
        statusList.push({ label: 'Picked up', date: gaudaan.statusHistory.find(h => h.status === 'picked_up')?.timestamp || gaudaan.updatedAt });
    } else if (gaudaan.status === 'assigned') {
        statusList.push({ label: 'Assigned', date: gaudaan.statusHistory.find(h => h.status === 'assigned')?.timestamp || gaudaan.updatedAt });
    } else if (gaudaan.status === 'rejected') {
        statusList.push({ label: 'Rejected', date: gaudaan.statusHistory.find(h => h.status === 'rejected')?.timestamp || gaudaan.updatedAt });
    }
    ...
    const volunteerImg = gaudaan.assignedVolunteer?.profileImage
        ? `${import.meta.env.VITE_BACK_URL}/auth/profile/image/${gaudaan.assignedVolunteer?.profileImage}`
        : '/img/profile-img.webp';
    ...
    return (
        <div className="max-w-xl mx-auto p-4 space-y-6">
            ...
            <h2 className="text-2xl font-bold text-gray-800">{gaudaan.animalType} Gaudaan</h2>

            <div>
                <h3 className="text-lg font-semibold text-gray-700">Description</h3>
                <p className="mt-1 text-gray-600 p-3 bg-gray-50 rounded-lg border">{gaudaan.animalDescription || 'No description provided'}</p>
            </div>
            ...
            {gaudaan.assignedVolunteer && (
                <div>
                    <h3 className="text-lg font-semibold text-gray-700">Assigned Volunteer</h3>
                    <div className="bg-white shadow rounded-lg p-3 flex items-center gap-4 my-2">
                        <img src={volunteerImg} alt="Volunteer" className="h-12 w-12 rounded-full object-cover border" />
                        <div>
                            <p className="font-semibold">{gaudaan.assignedVolunteer.firstName} {gaudaan.assignedVolunteer.lastName}</p>
                            <p className="text-sm text-gray-600 flex items-center gap-1">
                ...
                <ol className="relative border-l-2 border-dashed border-green-600 ml-4 space-y-6">
                    ...
                    {statusList.length > 0 ? (
                        statusList.map((step, idx) => {
                            const isCurrent = step.label.toLowerCase().includes(gaudaan.status.replace(/_/g, ' '));
                            return (
                                <li key={idx} className="ml-4 relative">
                                    <div className={`absolute -left-7 flex items-center justify-center w-6 h-6 rounded-full text-sm font-semibold
                                 ${isCurrent ? 'bg-green-600 text-white' : 'bg-green-500 text-white'}`}>
                                        {idx + 1}
                                    </div>
                                    <div className={isCurrent ? "bg-blue-50 p-1 rounded-md" : ""}>
                                        <p className={`text-base font-semibold ${isCurrent ? 'text-green-600' : 'text-green-600'}`}>
                                            {step.label}
                                        </p>
            ...
            {showRejectConfirm && (
                <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
                    <div className="bg-white rounded-lg p-6 w-full max-w-sm text-center">
                        <h3 className="text-lg font-semibold mb-4">Are you sure you want to reject this gaudaan?</h3>
