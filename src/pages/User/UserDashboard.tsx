...
interface AuthContextType {
  ...
  logout: () => void;
  user: User | null;
  setUser?: React.Dispatch<React.SetStateAction<User | null>>; // Made optional
}

// ----- Component -----
const UserDashboard: React.FC = () => {
  const { authorizationToken, logout, user, setUser } = useAuth() as AuthContextType;
  ...
  const handleJoinVolunteer = async () => {
    try {
      ...
      const { user: updatedUser }: { user: User } = await res.json();
      if (setUser) {
        setUser(updatedUser); // Check if setUser exists before calling
      } else {
        console.warn("setUser is not available in auth context");
      }
      setIsModalOpen(false);
      toast.success("You joined as a volunteer successfully.");
      toast.info("Please re-login to apply changes.");
      logout();
    } catch (error) {
      console.error("Failed to become volunteer:", error);
      toast.error("Failed to join as a volunteer. Please try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        const countData: { counts: Record<string, number> } = await countRes.json();
