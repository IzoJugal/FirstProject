...
function CreateGaudaan() {
  const { authorizationToken } = useAuth();
  const navigate = useNavigate();
  const [isAnimalRegistered, setIsAnimalRegistered] = useState(false);
  const [imagePreviews, setImagePreviews] = useState([]);
  const [globalError, setGlobalError] = useState('');
  const [loading, setLoading] = useState(false);

  const { register, handleSubmit, formState: { errors }, setValue, watch, reset, setError, clearErrors } = useForm({
    defaultValues: {
      name: '',
      email: '',
      phone: '',
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        ...
        const { data } = await axios.get(`${import.meta.env.VITE_BACK_URL}/auth/auth`, {
  ...
  useEffect(() => {
    if (imageFiles.length === 0) {
      setError('images', { type: 'required', message: 'At least one image is required' });
    } else {
      clearErrors('images');
    }
  ...
  const onSubmit = async (data) => {
    ...
    // Simple validation for animalDescription
    if (!data.animalDescription) {
      setError('animalDescription', { type: 'required', message: 'Animal description is required' });
      toast.error('Animal description is required.');
      return;
    }
    if (data.animalDescription.length < 10) {
      setError('animalDescription', { type: 'minLength', message: 'Description must be at least 10 characters' });
      toast.error('Description must be at least 10 characters.');
      return;
    }
    if (data.animalDescription.length > 500) {
      setError('animalDescription', { type: 'maxLength', message: 'Description cannot exceed 500 characters' });
    ...
    try {
      ...
      Object.keys(data).forEach((key) => {
        if (key === 'images') {
          ...
          const [hourStr, minuteStr] = data[key].split(':');
          const hour = parseInt(hourStr, 10);
          const minute = parseInt(minuteStr, 10);
          const ampm = hour >= 12 ? 'PM' : 'AM';
          const hour12 = hour % 12 || 12;
          const formattedTime = `${hour12}:${minute.toString().padStart(2, '0')} ${ampm}`;
          formData.append('pickupTime', formattedTime);
        } else {
          formData.append(key, data[key]);
        }
      });
      await axios.post(`${import.meta.env.VITE_BACK_URL}/auth/gaudaan`, formData, {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-blue-50 flex items-center justify-center p-4">
      ...
      >
        ...
        {globalError && (
          <p className="text-center text-red-600 bg-red-50 p-3 rounded-lg">{globalError}</p>
        )}
        ...
        {/* Government ID */}
        <div>
          <label className="block text-sm font-semibold text-gray-700 mb-1">Government ID (Optional)</label>
          <input
            {...register('governmentId', {
              pattern: {
                value: /^\d{12}$|^$/,
                message: 'Government ID must be a 12-digit number or left blank',
              },
            })}
            type="text"
            maxLength={12}
          ...
          />
          {errors.governmentId && <p className="text-sm text-red-500 mt-1">{errors.governmentId.message}</p>}
        </div>
        ...
        <div className="flex items-center space-x-3">
          <input
            ...
            onChange={(e) => setIsAnimalRegistered(e.target.checked)}
            className="w-5 h-5 text-green-600 border-gray-300 rounded focus:ring-green-500"
          />
          <label htmlFor="isAnimalRegistered" className="text-sm font-semibold text-gray-700">
        ...
        {isAnimalRegistered && (
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-1">Animal Registered ID</label>
            <input
              {...register('animalRegisteredId', { required: 'Animal Registered ID is required' })}
              type="text"
              className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 transition"
              placeholder="Enter Animal Registration ID"
            />
            {errors.animalRegisteredId && <p className="text-sm text-red-500 mt-1">{errors.animalRegisteredId.message}</p>}
          </div>
        )}
        ...
        <div>
          ...
          {errors.animalType && <p className="text-sm text-red-500 mt-1">{errors.animalType.message}</p>}
        </div>

        {/* Animal Condition */}
        <div>
          <label className="block text-sm font-semibold text-gray-700 mb-1">Animal Condition</label>
          <select
            {...register('animalCondition', { required: 'Animal condition is required' })}
            className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 transition"
          >
          ...
          {errors.animalCondition && <p className="text-sm text-red-500 mt-1">{errors.animalCondition.message}</p>}
        </div>

        {/* Animal Description */}
        <div>
          <label className="block text-sm font-semibold text-gray-700 mb-1">Animal Description</label>
          <textarea
          ...
          />
          {errors.animalDescription && <p className="text-sm text-red-500 mt-1">{errors.animalDescription.message}</p>}
        </div>

        {/* Image Upload */}
        <div>
          <label className="block text-sm font-semibold text-gray-700 mb-1">
            Upload Images (Max {MAX_IMAGES}, {MAX_SIZE_MB}MB each, at least 1 required)
          </label>
          <div
            {...getRootProps()}
            className={`w-full p-6 border-2 border-dashed ${
              isDragActive ? 'border-green-600 bg-green-100' : 'border-green-400 bg-green-50'
            } rounded-xl text-center cursor-pointer hover:bg-green-100 transition`}
          >
            <input {...getInputProps()} />
            <p className="text-gray-600">
              {isDragActive ? 'Drop the images here' : 'Drag & drop or click to select images'}
            </p>
          </div>
        ...
        <div>
          <label className="flex items-center space-x-2">
            <input
              {...register('consent', { required: 'You must consent to data processing' })}
              type="checkbox"
              className="w-5 h-5 text-green-600 border-gray-300 rounded focus:ring-green-500"
            />
            <span className="text-sm text-gray-700">
              I consent to the processing of my personal data as per the{' '}
              <a href="/privacy-policy" target="_blank" className="text-green-600 hover:underline">
                privacy policy
              </a>
            </span>
          </label>
